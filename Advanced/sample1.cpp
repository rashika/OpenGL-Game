...
void handleKeypress(int key,int x,int y)
{
   // keypressright=0;
	...
	current_tile.h=cor_y-0.75+inc+0.2;
	if(person_on_block())
	{
		cout<<"IN"<<endl;
	}
...
void camera()
{
	...
	if(ca==4)
	{	
		gluLookAt(cor_x,cor_y+0.75,cor_z-2.0,cor_x,cor_y,cor_z,0.0,1.0,0.0);
	}
}
void drawScene() {
...
bool person_on_block()
{
	...
	for(int i=0;i<tilesup.size();i++)
	{
		if(intersection()&&((abs(current_tile.h-tilesup[i].h)<=0.06)||flagblock))
		{
			...
			cor_y=current_tile.h+0.75-inc-0.2;
			flagblock=true;
			return true;
		}
	}
