...
class Person
{
	public:
		...
		GLMmodel* Still;
		bool UnderSpot;
		GLMmodel* Left;GLMmodel *Right;GLMmodel *Battery;GLMmodel *Diamond;
		GLMmodel* PowerP;
	...
	FOR(i,0,num_rows)
	{
		...
		/*	if(Rocks[num])
			...
			if(Collectible_Position==num)
			{
				...
				Goku.DrawGroup(Goku.Diamond,GLM_SMOOTH | GLM_TEXTURE | GLM_MATERIAL);
				glTranslatef(0.0f,-(1.0 + Raise_Value[num]),0.0);
			...
			if(Powerup_Pos==num)
			{
				...
				Goku.DrawGroup(Goku.PowerP,GLM_SMOOTH | GLM_TEXTURE | GLM_MATERIAL);
				glPopMatrix();
...
void Person :: SpotLight(void)
{
	...
	GLfloat lightPos0[]={ Cor_x - 0.3*sin(_angle*PI/180), Cor_y+1.8, Cor_z - 0.5*cos(_angle*PI/180), 1.0};
...
void Person :: LoadImages()
{
	...
	if(!Right)
		exit(0);
	DefineGroups(Right);
	strcpy(temp,"Diamond.obj");
	Diamond= glmReadOBJ(temp);
	DefineGroups(Diamond);
	strcpy(temp,"Battery.obj");
	PowerP= glmReadOBJ(temp);
	DefineGroups(PowerP);
}/*
...
//
	glTranslatef(1.5*sin(_angle*PI/180),0.0,1.5*cos(_angle*PI/180));
	glEnable(GL_COLOR_MATERIAL);
	glColor3f(1.0,1.0,0.0);
	gluCylinder(Keoken,0.4,0.4,val,32,32);
	float dist1 = Cor_x - (Cor_x + val*sin(Goku._angle*PI/180));
	dist1*=dist1; 
	dist1+=(Cor_z - (Cor_z + val*cos(Goku._angle*PI/180)))*(Cor_z - (Cor_z + val*cos(Goku._angle*PI/180)));
	dist1=sqrt(dist1);
	float dist2 = Cor_x - ECor_x;
	dist2*=dist2;
	dist2+=(Cor_z - ECor_z)*(Cor_z - ECor_z);
	dist2 = sqrt(dist2);
	float dist3 = ECor_x - (Cor_x + val*sin(_angle*PI/180));
	dist3*=dist3;
	dist3+=(ECor_z - (Cor_z + val*cos(_angle*PI/180)))*(ECor_z - (Cor_z + val*cos(_angle*PI/180)));
	dist3=sqrt(dist3);
	if(fabs(dist1-(dist2+dist3))<=0.2)
	{
			Goku.Enemy = true;
	}
	glPopMatrix();
...
{
	gluLookAt(Goku.Cor_x ,Goku.Cor_y + 1.0,Goku.Cor_z,Goku.Cor_x + 1.5*sin(Goku._angle*PI/180),Goku.Cor_y+1.0,Goku.Cor_z+1.5*cos(Goku._angle*PI/180),0,1,0);
}

void Person :: ThirdPerson()
{

	gluLookAt(Goku.Cor_x - 1.5*sin(Goku._angle*PI/180),Goku.Cor_y + 1.0,Goku.Cor_z - 1.5*cos(Goku._angle*PI/180),Goku.Cor_x + 1.5*sin(Goku._angle),Goku.Cor_y+1.0,Goku.Cor_z+1.5*cos(Goku._angle),0,1,0);
}
/*
...
void update(int value)
{
	...
	if(Current_Stage>=1)
	{
		if(Lvl1[Current_Stage].Clock&&Goku.Cor_x == Lvl1[Current_Stage].xi && Goku.Cor_z == Lvl1[Current_Stage].zi)
		{
			...
			if(Lvl1[Current_Stage].Timer>=200)
			{
				...
				Lvl1[Current_Stage].Raise_Value[Lvl1[Current_Stage].Cur_Tile]-=0.5;
				Goku.Cor_y-=0.5;
				Goku.iterate+=1;	
				if(Goku.Cor_y<=-40.0)
				{
		...
		else if(!Lvl1[Current_Stage].Clock)
		{
			...
			{
				Lvl1[Current_Stage].Clock = true;
				Lvl1[Current_Stage].Raise_Value[Lvl1[Current_Stage].Cur_Tile] = 0.0f;
				Lvl1[Current_Stage].InvisibleTiles[Lvl1[Current_Stage].Cur_Tile] = false;
			}
		}
		else 
	...
	if(Lvl1[Current_Stage].CollectibleTimer==250)
	{
		Lvl1[Current_Stage].Collectibles();
